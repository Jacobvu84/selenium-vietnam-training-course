...
def test_create_provider():
    ...
    mkdir("bin")
    setup_logger('provider', TEMP_DIR_WIN + 'bin\\provider.log', level=logging.DEBUG)
    ...
    try:
        ...
        for provider in providers[0:3]:
            log.info(
                ...
                new_pass + "," +
                provider.license.dea + "," +
                provider.license.npi + ",[end]")

            """[TEST] CREATE NEW PROVIDERS """
            create_provider(provider)

            with MailBox(main_email, email_pwd) as inbox:
                _mailbox.open_link_from_email(mail_title=_mailbox.welcome_provider, mbox=inbox)
